...
export const deleteExpense = async (id) => {
  return await axiosProvider
    .delete(ENDPOINTS.getExpenseById(id))
    .then((response) => {
...
export const saveExpenseChanges = async (expenseDataUpdate, idExpense) => {
  ...
  const response = await axiosProvider
    .patch(ENDPOINTS.getExpenseById(idExpense), expenseDataUpdate)
    .then((res) => {
...
export function validateDataToSave(expenseDataUpdate) {
  if (
    !expenseDataUpdate.date ||
    expenseDataUpdate.date == "" ||
    !expenseDataUpdate.expenseCategory ||
    expenseDataUpdate.expenseCategory == "" ||
    !expenseDataUpdate.description ||
    expenseDataUpdate.description == "" ||
    !expenseDataUpdate.paymentType ||
    expenseDataUpdate.paymentType == "" ||
    !expenseDataUpdate.amountSpend ||
    expenseDataUpdate.amountSpend === "" ||
    (expenseDataUpdate.expenseCategory === "STOCK" &&
      (expenseDataUpdate.quantity === "" || !expenseDataUpdate.fkProduct))
  ) {
