...
const CardSubJob = React.memo(({ data, onEdit, onUpdateStatus }) => {
  ...
  const handleChangeStatus =  async () => {
    ...
    const response = await updateSubJobStatus(data.id, data.status === "CLOSED" ? "PENDING" : "CLOSED", currentDateTime);
  ...
  return (
    ...
    >
      <div className="flex py-3 px-3 text-2xl font-bold items-center gap-[4px]">
      ...
      <div>
        <ul
          className={`py-1 text-[14px] pl-8 list-disc pr-2
                ${
                  data.status === "CLOSED" ? "marker:text-pink-zero" : "marker:text-cyan-zero"
                } marker:duration-100 ease-in-out`}
        >
          <li>
            <b>Descrição: </b> <span className="max-h-[3rem] overflow-y-auto break-words breakable-text"> {data?.description || ""} </span>
          </li>
          <li>
          ...
          <li>
            <b>Data Prevista: </b> <span className="breakable-text overflow-y-auto max-h-[3rem]"> {formatDateWithoutTime(data?.date) || ""} </span>
          </li>
          <li>
            <b>Status: </b> 
            <b className={`${data.status === "CLOSED" ? "text-cyan-zero":"text-yellow-zero"}`}>{data.status === "CLOSED" ? "Concluído" : "Pendente"}</b>
          </li>
          <li>
      ...
      <div className="flex justify-between w-[100%] pr-1 py-2 px-3 border-t-[#ccc] items-center">
          ...
          <div className="flex justify-between w-[70%] max-w-[15rem] gap-2 mr-2">
              <button
                ...
                className={`h-[34px] w-[7rem] text-[14px]  hover:text-[#B9B9B9]
                  font-bold cursor-pointer duration-100 ease-in-out
                  ${data.status === "CLOSED" ? "text-pink-zero  " : "text-cyan-zero "}
                  `} 
              >
